...
st.set_page_config(page_title="RiceGenix AI", layout="wide")
...
data['Yield'] = (
    2 + data['Gene_A']*0.9 + data['Gene_B']*0.8 +
    data['Gene_C']*0.7 + data['Gene_D']*0.6 +
    data['Rain']*0.012 - (data['Temp']-30)*0.06 +
    - abs(data['pH'] - 6.5)*0.5 +
    np.random.normal(0,0.3,data_size)
)

X = data[['Gene_A','Gene_B','Gene_C','Gene_D','Rain','Temp','pH']]
y = data['Yield']
...
with st.form("prediction_form"):
    ...
    height = st.number_input("Enter plant height (in inches)", min_value=0.0)

    gene_b = st.radio("Disease resistant?", ["Yes","No"])
    gene_c = st.radio("Drought tolerant?", ["Yes","No"])
    gene_d = st.radio("Fast growth?", ["Yes","No"])

    rain = st.number_input("Annual Rainfall (mm)", min_value=0.0)
    
    temp_unit = st.selectbox("Temperature Unit", ["Celsius", "Fahrenheit"])
    temp_input = st.number_input("Average Temperature")

    submit = st.form_submit_button("Predict Yield")

# ---------------- SOIL QUESTIONS (NEW) ----------------
st.subheader("Soil Conditions (for pH estimation)")
...
if manual_ph:
    ph = st.slider("Enter Soil pH", 3.0, 9.0, 6.5, step=0.1)
else:
    ph = 6.5
...
# ---------------- IMAGE UPLOAD ----------------
uploaded_file = st.file_uploader("Upload rice crop image (optional)", type=["jpg","png"])
...
if uploaded_file:
    ...
    # 🔥 REAL AI MODEL
    try:
        disease_name = predict(image, model_dl)
    except:
...
treatment_map = {
    ...
    "Brown Spot": "Apply Mancozeb fungicide, improve drainage and potassium nutrition",
    "Leaf Blast": "Use Tricyclazole fungicide, avoid excess nitrogen fertilizer",
    "Leaf Scald": "Use resistant varieties and apply systemic fungicides",
    "Narrow Brown Leaf Spot": "Apply Carbendazim or Propiconazole, maintain field hygiene",
    "Rice Hispa": "Apply Chlorpyrifos or Imidacloprid, remove affected leaves",
    "Sheath Blight": "Apply Validamycin or Hexaconazole, reduce plant density",
    "Healthy": "Crop is healthy, maintain irrigation and balanced fertilization",
    "Model Error": "Model not working properly"
}
...
if st.button("Predict Yield", use_container_width=True):
    ...
    # ---------------- BASE PREDICTION ----------------
    base_pred = model.predict([[g1, g2, g3, g4, rain_val, temp_val, ph]])[0]
    ...
    if height_val < expected_height:
        ...
        final_pred -= 0.3
        height_flag = "Overgrowth → possible nutrient imbalance"
    ...
    if disease_name != "Healthy" and disease_name != "Not Checked":
        final_pred -= 1.2

    # mismatch (important smart logic)
...
if st.session_state.result:
    ...
    st.write(f"Soil pH (Estimated): {res['ph']:.2f}")
    ...
    if res['rain'] < 100:
        ...
        st.write("Moderate low rainfall → maintain regular irrigation")
    ...
    # ---------------- TEMPERATURE ----------------
    if res['temp'] > 40:
        st.write("High temperature stress → maintain water level in field")

    elif res['temp'] < 20:
        st.write("Low temperature → growth may slow, monitor crop carefully")

    # ---------------- GENERAL IMPROVEMENT ----------------
    st.write("Use high-yield variety seeds like Swarna or IR64")
    st.write("Apply balanced NPK fertilizers based on soil test")
    st.write("Monitor crop weekly for early disease detection")
    ...
    # user point
    ax1.scatter(res['rain'], res['yield'], s=100)
    ...
    ax2.scatter(res['temp'], res['yield'], s=100)
    ...
    if st.button("Export Report as PDF"):
        ...
        content.append(Paragraph(f"<b>Temperature:</b> {res['temp']} °C", styles['Normal']))
        ...
        if uploaded_file:
            ...
            content.append(RLImage(temp_img.name, width=250, height=180))
